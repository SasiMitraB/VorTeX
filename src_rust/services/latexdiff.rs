...
/// The document to diff: the active file if it is a full document (or names one via
/// `%!TEX root`), else `main.tex`, else the first root-level file with `\documentclass`.
pub fn find_main_document(active: Option<&Path>, project: &Path) -> Option<PathBuf> {
    let is_tex = |p: &Path| p.extension().map(|e| e.eq_ignore_ascii_case("tex")).unwrap_or(false);
...
fn magic_root(text: &str) -> Option<String> {
    text.lines().take(20).find_map(|l| {
        ...
        let rest = l.strip_prefix("TEX")?.trim_start().strip_prefix("root")?.trim_start().strip_prefix('=')?;
        let root = rest.trim();
