use vortex_core::pdf_renderer::{get_pdf_page_count, get_pdf_page_dimensions, ensure_pdf_rendered, is_cache_valid, clear_pdf_cache};
use vortex_core::synctex::{synctex_forward_search, synctex_inverse_search, is_synctex_available};
...
";
        ...
        let res = vortex_core::compiler::build_latex_project(tex);
        if !res.success {
    ...
    let hl = vortex_core::synctex::SynctexRect::from(&fwd);
    println!("hl page {} x {} y {} w {} h {}", hl.page, hl.x, hl.y, hl.width, hl.height);
    assert_eq!(hl.page, 1);

    // Inverse at the middle of the highlight box should return the same file/line
    let inv = synctex_inverse_search(pdf, hl.page, hl.x + hl.width / 2.0, hl.y + hl.height / 2.0);
    println!("inverse {:?}", inv);
    assert!(inv.is_some());
    let inv = inv.unwrap();
