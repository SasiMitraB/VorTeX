...
/// Auxiliary files that TeX, BibTeX and latexmk write next to the document.
/// Builds keep them (for incremental runs), so the file tree hides them.
pub fn is_build_artifact(file_name: &str) -> bool {
    const SUFFIXES: &[&str] = &[
        ".aux", ".bbl", ".blg", ".fdb_latexmk", ".fls", ".log", ".out", ".toc", ".lof", ".lot",
        ".synctex.gz", ".synctex", ".synctex(busy)", ".nav", ".snm", ".vrb", ".bcf", ".run.xml",
        ".xdv", ".idx", ".ilg", ".ind", ".glo", ".gls", ".glg", ".ist", ".acn", ".acr", ".alg",
        ".loa", ".thm", ".nlo", ".nls",
    ];
...
pub fn build_tree(dir_path: &str) -> Vec<TreeNode> {
    ...
    if let Ok(entries) = fs::read_dir(path) {
        for entry_res in entries.flatten() {
            ...
            // Skip hidden files, node_modules, .git, and LaTeX's auxiliary files
            if file_name.starts_with('.') || file_name == "node_modules" || file_name == "target" || is_build_artifact(&file_name) {
                continue;
            }
...
pub fn get_or_create_pdf_thumbnail(pdf_path: &str) -> Option<String> {
    ...
    let path_hash = crate::pdf_renderer::hash_pdf_key(pdf_path);
