...
impl From<&SynctexForwardResult> for SynctexRect {
    fn from(r: &SynctexForwardResult) -> Self {
        // The box origin (h, v) is its baseline-left corner; the box extends `height` above it.
        let has_box = r.width > 0.0 && r.height > 0.0;
        let (x, y) = if has_box { (r.h, r.v - r.height) } else { (r.x, r.y - 12.0) };
        Self {
            page: r.page,
            x,
            y: y.max(0.0),
            width: if has_box { r.width } else { 80.0 },
            height: if has_box { r.height } else { 12.0 },
        }
    }
}
